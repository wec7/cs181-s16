...
centered such that $\forall x, \sum_i x_i = 0$. Let $X \in \mathbb{R}^{n \times
m}$ be the input matrix, the columns of which are the input vectors. We define:
...
\begin{itemize}
  ...
  \item[(c)] Suppose that $\lambda > 0$. Knowing that $J$ is a convex function
    ...
    \end{align}
    (Hint: you can state the relevant properties of the function $J$ without
    proving them)
...
\subsection*{3. Modeling Changes in Congress [10pts]}
 The objective of this problem is to learn about linear regression with basis functions by modeling the average age of the US Congress. Download the file \verb|congress-ages.csv| from the course website.  It has two columns.  The first one is an integer that indicates the Congress number. Currently, the 114th Congress is in session. The second is the average age of that members of that Congress.  The data file looks like this:
\begin{csv}
...
Implement basis function regression with ordinary least squares with the above data. Some sample Python code is provided in \verb|linreg.py|.  Plot the data and regression lines for the simple linear case, and for each of the following sets of basis functions:
\begin{itemize}
...
\begin{itemize}
\item \textbf{v1.0} -- 28 January 2015 at 13:00
\item \textbf{v1.1} -- 29 January 2015 at 09:00. Removed extra $\lambda$ in $E_W(\boldw)$, minor formatting edits.
\end{itemize}
